...
module App =
    ...
    type Msg =
        | Select of Peg
        | Unselect
        | Move of Peg * Position

    let initModel = { board = []; pegs = Set.empty; selected = None}
    ...
    let update msg model =
        ...
        | Unselect -> { model with selected = None }, Cmd.none
        | Move (peg, pos)  -> { model with pegs = model.pegs |> Set.remove peg |> Set.add pos; selected= None }, Cmd.none
